...
describe('lion-pointing-frame', () => {
  describe('Positions of pointing frame', () => {
    ...
    describe('Right and Left', () => {
      ...
      it('should adjust its position when space available on right of the container', async () => {
        ...
        /* TODO: uncomment after adding feature of these position in getPosition
  ...
  describe('Spacing of pointy frame', () => {
    ...
    it('should adjust pointing-frame padding based on the position and invoker-width', async () => {
      const el = await fixture(html`
        <lion-popup position="right-of-top" style="margin-top: 240px; margin-left: 240px;">
          <lion-pointing-frame slot="content" class="popup pointing-frame">
            <div class="my-content">
              <div>Hello, Planet!</div>
            </div>
      ...
      expect(await updateSpacing('padding-left', el, pointingFrame, false)).to.equal('31px');

      el.position = 'left-of-top';
      expect(await updateSpacing('padding-right', el, pointingFrame)).to.equal('31px');
      ...
      el.position = 'right-of-bottom';
      expect(await updateSpacing('padding-left', el, pointingFrame)).to.equal('31px');
    ...
    // TODO: Add some more edge cases related to this kinds of things
    it('should add negative margin to the host if spacing is required is negative', async () => {
      const el = await fixture(html`
        <lion-popup position="top-of-right" style="margin-top: 240px; margin-left: 240px;">
          <lion-pointing-frame slot="content" class="popup pointing-frame">
            <div class="my-content">
              <div>Hello, Planet!</div>
            </div>
      ...
      await nextFrame();
      const lionPointingFrame = el.querySelector('lion-pointing-frame');
      expect(await updateSpacing('margin-bottom', el, el, false)).to.equal('-4px');
      ...
      el.position = 'top-of-left';
      expect(await updateSpacing('margin-bottom', el, lionPointingFrame)).to.equal('-4px');
    ...
    it('should add some extra pointing-frame padding based on pointer dimensions', async () => {
      const el = await fixture(html`
        <lion-popup position="center-of-top">
          <lion-pointing-frame slot="content" class="popup pointing-frame">
            <div class="my-content">
              <div>Hello, Planet!</div>
            </div>
      ...
      await nextFrame();
      const lionPointingFrame = el.querySelector('lion-pointing-frame');
      const icon = lionPointingFrame.querySelector('[slot="pointer"]');

      expect(await updateSpacing('padding-bottom', el, lionPointingFrame, false)).to.equal(
        icon.height,
      );

      el.position = 'center-of-bottom';
      expect(await updateSpacing('padding-top', el, lionPointingFrame)).to.equal(icon.height);

      el.position = 'center-of-right';
      expect(await updateSpacing('padding-left', el, lionPointingFrame)).to.equal(icon.height);

      el.position = 'center-of-left';
      expect(await updateSpacing('padding-right', el, lionPointingFrame)).to.equal(icon.height);
    });
  });
  ...
  describe('Position of pointer', () => {
    ...
    it('should adjust pointer icon position based on the position and invoker-width', async () => {
      ...
      const pointerIcon = el
        .querySelector('[slot="content"]')
        .shadowRoot.querySelector('lion-icon');
      ...
      el.position = 'center-of-left';
      expect(await updateSpacing('top', el, pointerIcon)).to.equal('12px');
      expect(await updateSpacing('right', el, pointerIcon)).to.equal('0px');

      el.position = 'center-of-right';
      expect(await updateSpacing('top', el, pointerIcon)).to.equal('12px');
      expect(await updateSpacing('left', el, pointerIcon)).to.equal('0px');
    });
  });
  ...
  describe('Passing a custom pointer', () => {
    it('should allow you to pass a custom pointer', async () => {
      ...
      expect(
        window.getComputedStyle(lionPointingFrame).getPropertyValue('padding-bottom'),
      ).to.equal('10px');

      const icon = el.querySelector('[slot="pointer"]');
      expect(icon).to.equal(customPointerSvg);
    });
    ...
    it('has a default pointer', async () => {
      ...
      const lionPointingFrame = el.querySelector('lion-pointing-frame');
      const icon = lionPointingFrame.querySelector('[slot="pointer"]');
      expect(icon).to.equal(pointerSvg);
    });
  });
});
